...
public class GyroITG3200 extends SensorBase implements PIDSource,
	...
	public GyroITG3200(I2C.Port port) {
		...
		// TODO: This report is incorrect. Need to create instance for I2C
		// ITG3200 Gyro
	 ...
	 * want it to enter standby mode, or another less demanding mode of
	 * operation. This also sets the gyroscope to use the X-axis gyro for a
	 * clock source. Note that it doesn't have any delays in the routine, which
	 * means you might want to add ~50ms to be safe if you happen to need to
	 * read gyro data immediately after initialization. The data will flow in
	 * either case, but the first reports may have higher error offsets.
	 */
	...
	private void writeBit(int register, byte bit, boolean value) {
		...
		if (DEBUG) {
			...
			if (newValue != buf[0]) {
				System.out
						.println("Expected " + newValue + " seeing " + buf[0]);
			}
		}
	}
	...
	public static byte updateByte(byte original, int bit, int numBits,
		 ...
		 * ); System.out.println( "" ); System.out.println( "mask           = "
		 * + GetBinaryString(mask) ); System.out.println( "maskedOriginal = " +
		 * GetBinaryString(maskedOriginal) ); System.out.println(
		 * "shifted Value  = " + GetBinaryString(shiftedValue) );
	...
	private static byte getBits(byte bitField, int bit, int numBits) {
		 ...
		 * GetBinaryString(maskedInput) ); System.out.println(
		 * "result         = " + GetBinaryString(result) );
		 */
	...
	public void setDeviceID(byte id) {
		writeBits(ITG3200_RA_WHO_AM_I, ITG3200_DEVID_BIT, ITG3200_DEVID_LENGTH,
				id);
	}
	 ...
	 * then filtered digitally and delivered into the sensor registers after the
	 * number of cycles determined by this register. The sample rate is given by
	...
	// DLPF_FS register
	/**
	 * Full-scale range. The FS_SEL parameter allows setting the full-scale
	 * range of the gyro sensors, as described in the table below. The
	 * power-on-reset value of FS_SEL is 00h. Set to 03h for proper operation.
	 *
	...
	/**
	 * Get "device ready" interrupt enabled setting. Will be set 0 for disabled,
	 * 1 for enabled.
	 * 
	...
	/**
	 * Get Device Ready interrupt status. The ITG_RDY interrupt indicates that
	 * the PLL is ready and gyroscopic data can be read.
	 * 
	...
	/**
	 * Get Data Ready interrupt status. In normal use, the RAW_DATA_RDY
	 * interrupt is used to determine when new sensor data is available in and
	 * of the sensor registers (27 to 32).
	 * 
	...
	public boolean getIntDataReadyStatus() {
		return readBit(ITG3200_RA_INT_STATUS,
	...
	public byte getClockSource() {
		...
		// I2Cdev::readBits(devAddr, ITG3200_RA_PWR_MGM,
		// ITG3200_PWR_CLK_SEL_BIT, ITG3200_PWR_CLK_SEL_LENGTH, buffer);
		return (byte) (buf[0] & ITG3200_PWR_CLK_SEL_BIT);
	}

	/**
	 * Set clock source setting. On power up, the ITG-3200 defaults to the
	 * internal oscillator. It is highly recommended that the device is
	 * configured to use one of the gyros (or an external clock) as the clock
	 * reference, due to the improved stability.
	 *
	 ...
	 * reference 5 | PLL with external 19.2MHz reference 6 | Reserved 7 |
	 * Reserved
	 *
	 * @param source
	...
	public double pidGet() {
		// TODO We likely want to return one of the axes based on a setup
		// option.
	...
	public static final byte ITG3200_ADDRESS_AD0_LOW = 0x68; // address pin low
																...
																// Digital Combo
																// board
	public static final byte ITG3200_ADDRESS_AD0_HIGH = 0x69; // address pin
																// high (VCC),
																// default for
																// SparkFun
																// ITG-3200
